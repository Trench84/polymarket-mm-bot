...
def _attempt_redemption(
    ...
    just-closed-window redemption and the pending-retry sweep below."""
    try:
...
async def run(config: Config, flag_path: Path, redemption_state_path: Path) -> None:
    order_client = (
        DryRunClobClient()
        if config.dry_run
        else LiveClobClient(
    ...
    while True:
        ...
        if window is None or new_start != window.start_ts:
            ...
            if previous_window is not None and should_attempt_redemption(inventory.up_shares, inventory.down_shares):
                _attempt_redemption(redeemer, retry_queue, previous_window.condition_id, previous_window.slug)
            ...
            for condition_id in retry_queue.pending():
                _attempt_redemption(redeemer, retry_queue, condition_id, condition_id)
...
def main() -> None:
    ...
    asyncio.run(run(config, args.flag_file, args.redemption_state_file))
